...
SELECT id FROM orders WHERE customer_id = (SELECT id FROM customers WHERE fname='Jane');
...
SELECT customers.email FROM customers LEFT JOIN orders ON customers.id = orders.customer_id GROUP BY customers.email HAVING SUM(num_cupcakes) > 100 ORDER BY email DESC;
...
SELECT DISTINCT customers.fname, customers.lname, customers.email FROM orders LEFT JOIN customers ON orders.customer_id = customers.id WHERE orders.processed='t'AND orders.cupcake_id = (SELECT cupcakes.id FROM cupcakes WHERE cupcakes.name='funfetti');